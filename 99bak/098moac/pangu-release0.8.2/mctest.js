/*
 * Example JS program to execute on MOAC
*/
var cache = [
  '',
  ' ',
...
function FutureSend(src, passwd, target, value, block, revertable)
{
	...
	var lockstr = leftPad(chain3.toHex(lock).slice(2).toString(16),64,0);
	var hashstr = leftPad(hash.substring(2),64,0);
	var unlockstr = leftPad(src.substring(2),64,0);
	
	var strData = "0xdccf0f61";
	strData = strData + blkstr + strtgt + amt + revertstr +lockstr + hashstr + unlockstr;
